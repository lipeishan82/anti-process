...
public class JdbcTest {
	...
	public void findByIdAndTime(int ids, int date) throws SQLException {
		...
		try {
			...
			stmt = conn.createStatement();
			// 执行SQL语句
			String sql = "select * from anti_mes  where id= '" + ids + "' and DATE_FORMAT(Dtime,'%Y%m%d') = '" + date
					+ "' order by Dtime";
			try {
				rs = stmt.executeQuery(sql);
			} catch (SQLException e) {
	...
	// 插入语句
	public boolean insert(int ids, String name, String sex, String area, String sy0, String sy1, String sy2,
			String sy3) {
	...
	public void GET(ResultSet rs) {
		...
		try {
			...
			while (rs.next()) {
				...
				check = rs.getString("sy0");
				sym = rs.getString("sy1");
				wuhan = rs.getString("sy2");
				touch = rs.getString("sy3");
				time = rs.getDate("Dtime");
				PRINT(id, name, sex, area, check, sym, wuhan, touch, time);
				ans++;
			}
			if (ans == 0) {
				System.out.println("\n当前信息暂无匹配项!");
	...
	// 信息显示
	public void PRINT(int id, String name, String sex, String area, String check, String sym, String wuhan, String touch,
			Date time) {
		System.out.print(id + "\t" + name + "\t " + sex + "\t" + area + "\t");
		System.out.print("   " + check + "\t           " + sym + "     \t     " + wuhan + "\t       \t\t" + touch + "\t\t");
